...
class penjualan
{
	...
	{
		$this->con=new mysqli("localhost","root","","kasirku");
	}
	function __destruct()
	{
		$this->con->close();
	...
	function tambah_barang($nama_barang,$stok,$harga_beli,$harga_jual,$id_kategori){
		...
		if ($query->num_rows > 0) {
			...
			if ($query2===TRUE) {
				$this->alert("Menu Berhasil Ditambahkan");
				$this->redirect("menu.php");
			}
			else{
				$this->alert("Menu Gagal Ditambahkan");
				$this->redirect("menu.php");
			}
		}
	}
